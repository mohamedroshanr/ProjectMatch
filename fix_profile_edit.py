...
# Update display names logic to use states if defined, else fallback
update_logic = """
...
"""
content = re.sub(
    r'const mockUser = MOCK_USERS\.find\(u => u\.id === id\);\n\s*const displayName = mockUser \? mockUser\.name : "User";\n\s*const displayHandle = "@" \+ displayName\.toLowerCase\(\)\.replace\(\/\\s\+\/g, \'_'\);\n\s*const displayAvatar = mockUser \? mockUser\.avatar : `https:\/\/api\.dicebear\.com\/7\.x\/notionists\/svg\?seed=\$\{id\}&backgroundColor=b6e3f4`;',
    update_logic,
    content
)
...
header_ui = """
          <div className="px-8 pb-8 -mt-16 relative z-10 flex flex-col md:flex-row items-center md:items-end gap-6">
            <Avatar className="w-32 h-32 border-4 border-nexa-card-dark bg-nexa-card-dark">
